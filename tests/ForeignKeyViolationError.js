  ...
  describe('foreign key violation error', () => {
    tables(session, [
      ...
      },
      {
        ...
        build: (table) => {
          ...
          table.integer('foreignKey').unsigned().references('target.id').onDelete('CASCADE');
        },
      },
    ]);
    ...
    describe('insert', () => {
      it('snake_case column', () => {
        return Bluebird.resolve(knex(sourceTable).insert({ foreign_key: 123456 }))
          .reflect()
          .then((res) => {
            logError(res);
      ...
      it('camelCase column', () => {
        return Bluebird.resolve(knex(sourceTable).insert({ foreignKey: 123456 }))
          .reflect()
          .then((res) => {
            logError(res);
...
function isMysqlV2Error(session, error) {
  return session.isMySql() && error.nativeError.code.indexOf('_2') !== -1;
}
