...
const { isCode, getMessage } = require('../util');

const INSERT_UPDATE_REGEX =
  /The (?:INSERT|UPDATE) statement conflicted with the FOREIGN KEY constraint "(.+)". The conflict occurred in database "(.+)", table "(.+)\.(.+)", column '(.+)'./;
const DELETE_REGEX =
...
module.exports = {
  ...
  parse: (err) => {
    if (isCode(err, 16, 547)) {
      const insertUpdateMatch = INSERT_UPDATE_REGEX.exec(getMessage(err));
      ...
      const deleteMatch = DELETE_REGEX.exec(getMessage(err));
